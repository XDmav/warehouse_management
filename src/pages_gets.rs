...
use crate::useful_funcs::{
	check_permission, get_user, read_file_to_string, replace_in_html, SharedStateStruct,
};
...
pub async fn stats_goods(
	...
	for r in rows {
		...
		falling_goods.push_str(&format!("<li>{} — {} → {}</li>", name, prev, now));
	}
